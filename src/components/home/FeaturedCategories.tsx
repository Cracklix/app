...
/**
 * @fileOverview Institutional Category Hub v32.0 - Alignment Hardened.
 * FIXED: Strictly using flex flex-col h-full and mt-auto to ensure button alignment across rows.
 */
    ...
    <section className="py-8 md:py-24 bg-white border-t border-slate-50">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 space-y-5 md:space-y-16">
        ...
        <div className="space-y-1 md:space-y-2 text-left">
          ...
          <p className="max-w-2xl text-[13px] md:text-xl font-medium text-slate-500">Select your recruitment category to explore verified preparation hubs.</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-8 lg:gap-10">
          {catLoading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <Skeleton 
                key={i} 
                className="h-[270px] md:h-[400px] w-full rounded-xl md:rounded-[3rem] bg-slate-50" 
              />
            ))
            ...
            return (
              ...
              >
                 <Link href={`/exams/category/${cat.id}`} className="h-full block">
                    <Card className="border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 rounded-2xl md:rounded-[3rem] bg-white group overflow-hidden flex flex-col p-4 md:p-10 h-[270px] md:h-[400px]">
                       <div className="flex-1 flex flex-col justify-start">
                          <div className="space-y-2 md:space-y-3">
                             ...
                             <Badge className="bg-primary/5 text-primary border-none font-bold text-[8px] md:text-[13px] px-2 py-0.5 rounded-full shadow-sm w-fit tracking-tight">
                                {boardLabel} Hub
                             </Badge>
                             <h3 className="text-[13px] md:text-2xl font-black leading-tight text-[#0F172A] group-hover:text-primary transition-colors line-clamp-2">
                                {cat.title}
                             </h3>
                          </div>
                       </div>
                       ...
                       <div className="mt-auto shrink-0 pt-3 md:pt-4">
                          <Button variant="ghost" className="w-full h-10 md:h-14 rounded-full bg-[#0F172A] text-white group-hover:bg-primary transition-all font-semibold text-[11px] md:text-base gap-2 shadow-md border-none active:scale-95">
                             Open Hub
