...
/**
 * @fileOverview Canonical Punjab Categories Hub v30.0.
 * REBUILT: Strictly aligned to 8 top-level categories.
 */
...
export default function FeaturedCategories() {
  ...
  // Strictly filter for the 8 canonical IDs to prevent legacy data leakage
  const canonicalIds = [
    "cat-punjab-general",
    "cat-punjab-teaching",
    "cat-punjab-technical",
    "cat-punjab-banking",
    "cat-punjab-police",
    "cat-punjab-health",
    "cat-punjab-courts",
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-2">
             <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold text-slate-400 tracking-tight uppercase">Choose Your Category</span>
             </div>
             <h2 className="text-3xl md:text-5xl font-black text-[#0F172A] leading-tight">Exam Categories</h2>
          </div>
            ...
            <motion.div key={cat.id} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.05 }}>
               <Link href={`/exams/category/${cat.id}`}>
                  <Card className="border border-[#E5E7EB] shadow-sm hover:shadow-xl transition-all duration-500 rounded-[2rem] bg-white group overflow-hidden flex flex-col p-6 min-h-[220px]">
                     ...
                     <div className="mt-auto pt-6 flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-primary"><span>Open Category</span><ArrowRight className="h-3.5 w-3.5 group-hover:translate-x-1 transition-transform" /></div>
                  </Card>
               </Link>
            </motion.div>
          ))}
