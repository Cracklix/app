...
/**
 * @fileOverview Popular Exam Verticals v12.0 - Alignment Hardened.
 * FIXED: Strictly using flex flex-col h-full and mt-auto to ensure button alignment.
 */
                     ...
                     <Card className="border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 rounded-2xl md:rounded-[3rem] bg-white p-4 md:p-10 flex flex-col h-[200px] md:h-[340px] relative overflow-hidden group">
                        <div className="flex-1 flex flex-col justify-start">
                           <div className="space-y-2 md:space-y-4">
                              <div className="h-9 w-9 md:h-20 md:w-20 bg-slate-50 rounded-lg md:rounded-[2rem] shadow-inner flex items-center justify-center overflow-hidden p-1 group-hover:scale-110 transition-transform">
                                 <AuthorityLogo boardId={p.boardId} size="sm" className="h-full w-full" />
                              </div>
                              <div className="space-y-1">
                                <h3 className="text-[13px] md:text-2xl font-black tracking-tight text-[#0F172A] group-hover:text-primary transition-colors uppercase leading-tight">{p.name}</h3>
                        ...
                        <div className="mt-auto shrink-0 pt-3 md:pt-4">
                           <Button variant="ghost" className="w-full h-10 md:h-14 rounded-full bg-[#0F172A] text-white group-hover:bg-primary transition-all font-black text-[10px] md:text-xs tracking-widest uppercase border-none shadow-md active:scale-95 gap-2">
...
function MiniChip({ emoji, label }: { emoji: string, label: string }) {
   return (
      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-slate-50 border border-slate-100 text-[7px] md:text-[8px] font-black text-slate-400 uppercase tracking-tight">
