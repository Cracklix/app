...
/**
 * @fileOverview High-Fidelity "Continue Learning" Hub v7.0.
 * FIXED: Resolved navigation to results by ensuring correct ID mapping.
...
export default function ContinueLearning() {
  ...
  const resultsQuery = useMemo(() => {
    ...
    // Fetch latest 2 results for quick resume
  ...
  if (!recentAttempts || recentAttempts.length === 0) return null;
    ...
    <section className="space-y-6 md:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
         {recentAttempts.map((res: any) => {
            // Priority: Result Doc ID, Fallback: Mock ID
            const resultPath = `/results/${res.id || res.mockId}`;
                    ...
                    <div className="flex-1 min-w-0 space-y-4 md:space-y-6 text-left">
                       ...
                       <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-10">
                          <Button asChild className="h-11 md:h-14 px-6 md:px-8 bg-primary hover:bg-orange-600 text-white font-black uppercase text-[8px] md:text-[10px] tracking-[0.2em] rounded-xl md:rounded-2xl shadow-xl transition-all active:scale-95 border-none cursor-pointer">
                             <Link href={resultPath}>REVIEW RESULT <ArrowRight className="ml-2 h-3 w-3" /></Link>
                          </Button>
