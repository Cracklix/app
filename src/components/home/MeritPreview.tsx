...
/**
 * @fileOverview Merit Hub v8.4 - Normalized Case.
 */
    ...
    <section className="py-12 md:py-24 bg-slate-50/50 border-t border-slate-100 overflow-hidden">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 text-left">
        <div className="flex items-center justify-between mb-8 md:mb-16 px-1">
           ...
           <Button asChild variant="ghost" className="text-primary font-bold text-base tracking-tight gap-2">
              <Link href="/leaderboard" className="flex items-center gap-2">View Full List <ChevronRight className="h-4 w-4" /></Link>
           </Button>
        </div>
        ...
        <div className="flex overflow-x-auto no-scrollbar -mx-4 px-4 gap-4 md:grid md:grid-cols-2 lg:grid-cols-4 md:mx-0 md:px-0 scroll-smooth snap-x pb-4">
           {resultsLoading ? (
              ...
              return (
                 <Card key={res.id} className="min-w-[260px] md:min-w-0 snap-center border border-slate-100 shadow-xl rounded-[2.5rem] bg-white p-6 hover:shadow-2xl transition-all group overflow-hidden">
