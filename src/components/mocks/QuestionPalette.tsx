...
/**
 * @fileOverview Institutional CBT Matrix v17.0.
...
export default function QuestionPalette({ onSelect }: { onSelect: (index: number) => void }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white text-left font-body p-5">
            ...
            {visibleQuestions.map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-10 h-10 rounded-lg flex items-center justify-center font-black text-xs transition-all border-2",
                    isCurrent ? "border-primary bg-white text-primary scale-110 shadow-md z-10" : "border-transparent",
      ...
      {/* 3. PAGINATION HUB */}
      {totalPages > 1 && (
        <div className="mt-6 pt-3 border-t border-slate-100 flex items-center justify-between shrink-0">
           <button 
              disabled={currentPage === 0}
              onClick={() => setCurrentPage(p => p - 1)}
              className="h-9 w-9 flex items-center justify-center rounded-xl bg-slate-50 hover:bg-slate-100 disabled:opacity-30 transition-all border border-slate-200"
           >
...
function LegendItem({ count, label, color, textColor = "text-white", colSpan = 1 }: any) {
  return (
    <div className={cn("flex items-center gap-2 p-2 rounded-lg border border-slate-100 bg-white shadow-sm", colSpan > 1 && "col-span-2")}>
