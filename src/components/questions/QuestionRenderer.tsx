...
/**
 * @fileOverview Institutional High-Fidelity Dark Renderer v21.0.
 * Updated: Options implemented as white boxes with black text as per user request.
 * Selection: Orange border and glow for active nodes.
 */
export default function QuestionRenderer({ 
  ...
  return (
    <div className="w-full text-left font-body bg-[#111111] text-white p-6 md:p-12 rounded-[2.5rem] shadow-4xl min-h-[400px] flex flex-col select-none border border-white/5 transition-all">
      ...
      <div className="space-y-4 md:space-y-6 mb-10 md:mb-12">
         ...
         <div className="space-y-4 md:space-y-6">
            {(isEn || isBi) && englishQ && (
              <div className="font-[700] text-[18px] md:text-[24px] leading-snug tracking-tight text-white antialiased">
                <MathText text={englishQ} />
              </div>
            )}
            {(isPa || isBi) && punjabiQ && (
              <div className="font-[700] text-[18px] md:text-[24px] leading-snug tracking-tight text-slate-100 antialiased">
      ...
      {!hideOptions && (
        <div className="flex flex-col space-y-4 md:space-y-6 mb-10 md:mb-12">
          {['A', 'B', 'C', 'D'].map((key, idx) => {
            ...
            return (
              <div 
                ...
                className={cn(
                  "flex items-center gap-4 p-4 md:p-6 rounded-2xl cursor-pointer transition-all active:scale-[0.98] border-2",
                  isSelected 
                    ? "bg-white border-[#F97316] shadow-[0_0_20px_rgba(249,115,22,0.15)]" 
                    : "bg-white border-transparent hover:bg-slate-50"
                )}
              >
                <span className={cn(
                  "font-[900] text-[17px] md:text-[22px] shrink-0 transition-colors",
                  isSelected ? "text-[#F97316]" : "text-[#0F172A]"
                )}>
                  ({key})
                </span>
                <div className={cn(
                  "font-[700] text-[15px] md:text-[21px] leading-tight transition-colors text-[#0F172A]"
                )}>
      ...
      {showSolution && (
        <div className="mt-6 space-y-10 animate-in fade-in slide-in-from-top-4 duration-500">
           <div className="font-[900] text-[16px] md:text-[21px] text-emerald-400 bg-emerald-500/5 p-4 rounded-xl border border-emerald-500/20 inline-block">
           ...
           <div className="space-y-10 pt-10 border-t border-white/10">
              {(englishExp || isBi) && (
                <div className="flex gap-4 md:gap-6 items-start text-left">
                   <div className="h-2 w-2 rounded-full border-2 border-primary shrink-0 mt-2.5 md:mt-3 shadow-[0_0_8px_rgba(249,115,22,0.5)]" />
                   <div className="font-[700] text-[15px] md:text-[18px] leading-[1.6] md:leading-[1.8] text-slate-300 flex-1">
