...
interface QuestionRendererProps {
  question: Partial<Question> & { displayId?: string; tableData?: any; chartConfig?: any };
  language: 'en' | 'pa' | 'bilingual';
  showSolution?: boolean;
}

/**
 * @fileOverview High-Fidelity Question Renderer v6.0.
 * Optimized: Bilingual parity and absolute formatting preservation.
 */
...
export default function QuestionRenderer({ question, language, showSolution = false }: QuestionRendererProps) {
  ...
  // Robust field scanning for provided rationales
  const expEn = useMemo(() => {
      ...
      {showSolution && (
        <div className="mt-12 p-8 bg-emerald-50 rounded-[2.5rem] border border-emerald-100 space-y-6 shadow-xl">
           ...
           <div className="space-y-6 pt-6 border-t border-emerald-200/50">
              ...
              {!expEn && !expPa && (
                <div className="flex items-center gap-3 p-6 bg-white/40 rounded-[1.5rem] border border-dashed border-emerald-200">
                   <BrainCircuit className="h-5 w-5 text-emerald-400" />
                   <p className="text-[12px] font-bold text-emerald-600 uppercase tracking-tight">No detailed institutional rationale recorded for this node.</p>
                </div>
              )}
           </div>
        </div>
