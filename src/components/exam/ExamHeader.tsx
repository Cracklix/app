 ...
 * Optimized: Mobile bar refined to prevent clipping of tactical controls.
 */
export default function ExamHeader({ 
  ...
  return (
    <header className="bg-[#0B1528] text-white flex flex-col shrink-0 shadow-2xl z-[100] select-none border-b border-white/5">
      ...
      <div className="flex lg:hidden items-center justify-between px-3 h-9 border-b border-white/5 bg-black/40">
         ...
         <button onClick={onExitRequest} className="text-[7px] font-black uppercase text-rose-500 tracking-widest border border-rose-500/20 px-2 py-0.5 rounded ml-2 shrink-0">
           EXIT
         </button>
      </div>
      ...
      <div className="h-12 md:h-16 flex items-center justify-between px-3 md:px-8">
        ...
        <div className="flex items-center gap-1.5 md:gap-4">
           ...
           <Button 
             ...
             className="h-8 w-8 md:h-10 md:w-10 rounded-lg md:rounded-xl bg-white/5 text-white hover:bg-white/10 shrink-0 border border-white/5"
           >
             {isPaused ? <Play className="h-3.5 w-3.5 md:h-4 md:w-4 fill-current text-[#F97316]" /> : <Pause className="h-3.5 w-3.5 md:h-4 md:w-4 fill-current" />}
           </Button>
           
           <div className="flex flex-col items-start leading-none ml-1 md:ml-2">
              <p className="text-[6px] md:text-[7px] font-black uppercase text-slate-500 tracking-widest mb-0.5">PROGRESS</p>
              <p className="text-xs md:text-sm font-black text-white">
                 {currentIdx + 1}<span className="text-slate-500 text-[10px] md:text-xs font-bold">/{questions.length}</span>
              </p>
           </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 md:gap-5">
           ...
           <Button 
             ...
             className="bg-[#F97316] hover:bg-orange-600 h-8 md:h-12 px-3 md:px-6 rounded-lg md:rounded-xl font-black uppercase text-[8px] md:text-[10px] tracking-widest gap-1.5 md:gap-2 shadow-xl shrink-0"
           >
              <Menu className="h-3.5 w-3.5 lg:hidden" />
