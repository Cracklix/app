...
export default function Logo({ className = "", variant = 'light', showTagline = true }: LogoProps) {
  ...
  return (
    <Link href="/" className={`flex items-center gap-2 md:gap-3 group ${className}`}>
      <motion.div 
        whileHover={{ scale: 1.05 }}
        whileTap={{ scale: 0.95 }}
        className="relative w-8 h-8 md:w-10 md:h-10 flex items-center justify-center shrink-0"
      >
      ...
      <div className="flex flex-col min-w-0">
        <div className="flex items-baseline leading-none">
          <span className={`text-xl md:text-2xl font-black tracking-tighter ${isLightVariant ? 'text-white' : 'text-[#0F172A]'}`}>Crack</span>
          <span className="text-[#F97316] text-xl md:text-2xl font-black tracking-tighter">lix</span>
        </div>
        {showTagline && (
