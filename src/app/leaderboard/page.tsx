               ...
               <CardContent className="p-0">
                  ...
                  <div className="divide-y divide-slate-50">
                     ...
                     meritList.length > 0 ? meritList.map((entry: any, idx: number) => (
                          <div key={entry.id} className="p-8 md:p-10 grid grid-cols-12 items-center hover:bg-slate-50/50 transition-all group border-l-[4px] border-transparent hover:border-primary">
                             ...
                             <div className="col-span-2 text-center"><div className="inline-flex flex-col items-center"><span className="font-headline font-black text-xl md:text-4xl text-[#0F172A] tracking-tighter leading-none">{(entry.score || 0).toFixed(1)}</span><span className="text-[7px] md:text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">PTS</span></div></div>
...
function PodiumCard({ rank, data, color, isMain }: any) {
   ...
   return (
      <div className={cn("flex flex-col items-center space-y-8 group relative", isMain ? "mb-12 md:mb-16 z-20" : "mb-6 z-10")}>
         ...
         <div className="text-center space-y-3"><h3 className={cn("font-headline font-black text-[#0F172A] uppercase tracking-tight truncate max-w-[220px]", isMain ? "text-3xl md:text-4xl" : "text-xl md:text-2xl")}>{data?.name || '---'}</h3><div className="flex items-center justify-center gap-6"><div className="text-center"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">SCORE</p><p className="text-xl font-bold text-primary leading-none">{(data?.score || 0).toFixed(1)}</p></div><div className="h-6 w-px bg-slate-200" /><div className="text-center"><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">ACCURACY</p><p className="text-xl font-bold text-emerald-600 leading-none">{data?.accuracy || '0'}%</p></div></div></div>
      </div>
   )
}
