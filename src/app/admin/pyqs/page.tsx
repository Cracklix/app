...
/**
 * @fileOverview Institutional PYQ Archive CMS v17.0 (PWA Optimized).
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 px-1">
        <div className="space-y-1">
           <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Official Audit Archives</span>
           </div>
          <h1 className="text-2xl md:text-5xl font-black text-[#0F172A] tracking-tight">PYQ Registry</h1>
          <p className="text-slate-500 text-[11px] md:text-lg font-medium">Manage authentic previous year papers and official keys.</p>
        </div>
        <Button onClick={() => setEditingPYQ({ title: "", boardId: "", examId: "", year: new Date().getFullYear(), pdfUrl: "", isFree: true })} className="w-full md:w-auto h-11 md:h-14 px-8 bg-[#0F172A] hover:bg-black text-white font-black uppercase text-[10px] tracking-widest rounded-full shadow-xl border-none active:scale-95 gap-2">
          ...
          <Table className="min-w-[800px]">
            <TableHeader className="bg-slate-50/50">
              <TableRow className="border-slate-50 h-14 md:h-20">
                <TableHead className="px-6 md:px-12 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">Paper Identity</TableHead>
                <TableHead className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">Vertical hub</TableHead>
                <TableHead className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 text-center">Year</TableHead>
                ...
                <TableRow key={p.id} className="border-slate-50 hover:bg-slate-50 transition-colors group">
                  ...
                  <TableCell className="text-right px-6 md:px-12">
                     <div className="flex justify-end gap-2 md:gap-3 opacity-20 group-hover:opacity-100 transition-all">
                        <button onClick={() => setEditingPYQ(p)} className="h-9 w-9 md:h-11 md:w-11 rounded-xl bg-white shadow-sm border border-slate-100 flex items-center justify-center text-slate-400 hover:text-primary active:scale-90"><Edit className="h-5 w-5" /></button>
                        <button onClick={async () => { if(confirm("Purge paper node?")) await deleteDoc(doc(db!, "pyqs", p.id)) }} className="h-9 w-9 md:h-11 md:w-11 rounded-xl bg-white shadow-sm border border-slate-100 flex items-center justify-center text-rose-500 hover:bg-rose-50 active:scale-90"><Trash2 className="h-5 w-5" /></button>
                     </div>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Dialog open={!!editingPYQ} onOpenChange={o => !o && !isSaving && setEditingPYQ(null)}>
         <DialogContent className="sm:max-w-xl w-[95vw] max-h-[90vh] bg-white rounded-3xl md:rounded-[3rem] border-none shadow-5xl p-0 overflow-hidden text-left flex flex-col">
            ...
            <DialogHeader className="p-6 md:p-10 pb-2 md:pb-4 shrink-0">
               <DialogTitle className="text-xl md:text-3xl font-black uppercase">Archive Architect</DialogTitle>
               <DialogDescription className="text-slate-400 font-bold uppercase text-[9px] tracking-widest mt-1">Register official recruitment papers.</DialogDescription>
            </DialogHeader>
            <div className="px-6 md:px-10 pb-6 md:pb-10 space-y-6 md:space-y-8 overflow-y-auto custom-scrollbar flex-1">
               <div className="space-y-2 text-left">
                  ...
                  <Input value={editingPYQ?.title || ""} onChange={e => setEditingPYQ({...editingPYQ, title: e.target.value})} className="h-12 md:h-14 rounded-xl border-slate-200 bg-slate-50 font-bold" />
               </div>
               ...
               <div className="space-y-2 text-left">
                  <Label className="text-[9px] font-black uppercase text-slate-500 ml-1">Paper Year</Label>
               ...
               <div className="space-y-2 text-left">
                  <Label className="text-[9px] font-black uppercase text-slate-500 ml-1">PDF Registry URL</Label>
            ...
            <DialogFooter className="p-6 md:p-10 pt-4 bg-slate-50 border-t border-slate-100 flex flex-row gap-4">
               <Button variant="ghost" onClick={() => setEditingPYQ(null)} className="h-11 md:h-12 px-6 font-black uppercase text-[10px] text-slate-400">Cancel</Button>
               <Button onClick={handleSave} disabled={isSaving} className="flex-1 bg-[#0F172A] hover:bg-black text-white h-11 md:h-12 rounded-full font-black uppercase text-[10px] tracking-widest shadow-xl gap-2 active:scale-95 border-none">
