...
/**
 * @fileOverview Institutional Mock Builder Hub v21.0 (Unified Design System).
 */
...
function MockBuilderContent() {
  ...
  if (isInitializing) return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-[#0B1528] space-y-8">
       ...
       <p className="text-[10px] font-black uppercase text-primary">Hydrating Hub...</p>
    </div>
  );
    ...
    <div className="max-w-[1600px] mx-auto space-y-6 pb-40 text-left pt-4 px-4 md:px-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          <Button variant="ghost" size="icon" onClick={() => router.back()} className="rounded-full border bg-white h-12 w-12 shadow-sm">
        ...
        <Button onClick={handlePublish} disabled={isPublishing} size="lg" className="w-full md:w-auto gap-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
        <div className="lg:col-span-4 space-y-8">
           <Card className="border-none shadow-2xl rounded-[2.5rem] bg-white p-6 md:p-10 space-y-8 border border-slate-100">
              <div className="space-y-3">
                 <div className="text-[12px] font-black uppercase text-slate-500 ml-1">Series Headline</div>
                 <Input value={mockData.title} onChange={e => setMockData((p: any) => ({...p, title: e.target.value}))} className="h-14 rounded-full bg-slate-50/50 border-none font-black text-lg px-6 shadow-inner focus-visible:ring-primary text-[#0F172A]" placeholder="Patwari Mock Series 01" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-3">
                    <Label className="text-[12px] font-black uppercase text-slate-500 ml-1">Mock Type</Label>
                    <Select value={mockData.mockType} onValueChange={(v: MockType) => setMockData((p: any) => ({...p, mockType: v}))}><SelectTrigger className="h-11 md:h-12 rounded-full bg-slate-50/50 border-none font-black text-[12px] uppercase"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="FULL">Full Length</SelectItem><SelectItem value="SUBJECT">Subject-Wise</SelectItem><SelectItem value="SECTIONAL">Sectional</SelectItem><SelectItem value="PYQ">Official PYQ</SelectItem></SelectContent></Select>
                 </div>
                 <div className="space-y-3">
                    <Label className="text-[12px] font-black uppercase text-slate-500 ml-1">Access Level</Label>
                    <Select value={mockData.accessLevel} onValueChange={(v: AccessLevel) => setMockData((p: any) => ({...p, accessLevel: v}))}><SelectTrigger className="h-11 md:h-12 rounded-full bg-slate-50/50 border-none font-black text-[12px] uppercase"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="FREE">FREE HUB</SelectItem><SelectItem value="PREMIUM">ELITE HUB</SelectItem></SelectContent></Select>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-3">
                    ...
                    <Input type="number" value={mockData.duration} onChange={e => setMockData((p: any) => ({...p, duration: parseInt(e.target.value) || 120}))} className="h-11 md:h-12 rounded-full bg-slate-50/50 border-none font-black text-center" />
                 </div>
                 <div className="space-y-3">
                    <Label className="text-[12px] font-black uppercase text-slate-500 ml-1">Attempt Limit</Label>
                    <Input type="number" value={mockData.attemptLimit} onChange={e => setMockData((p: any) => ({...p, attemptLimit: parseInt(e.target.value) || 0}))} className="h-11 md:h-12 rounded-full bg-slate-50/50 border-none font-black text-center" />
                 </div>
              </div>
           </Card>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
           <div className="flex items-center gap-3 bg-white p-1.5 rounded-full border border-slate-100 shadow-sm w-fit">
              <button onClick={() => setActiveRightTab('BANK')} className={cn("px-8 py-2.5 rounded-full font-black uppercase text-[10px] tracking-tight transition-all", activeRightTab === 'BANK' ? "bg-blue-600 text-white shadow-xl" : "text-slate-400 hover:bg-slate-50")}>Registry Bank</button>
              <button onClick={() => setActiveRightTab('ASSEMBLY')} className={cn("px-8 py-2.5 rounded-full font-black uppercase text-[10px] tracking-tight transition-all", activeRightTab === 'ASSEMBLY' ? "bg-blue-600 text-white shadow-xl" : "text-slate-400 hover:bg-slate-50")}>Assembly Hub</button>
           </div>

           {activeRightTab === 'BANK' ? (
             <div className="space-y-8 animate-in zoom-in-95 duration-500">
                <Card className="bg-[#0B1528] rounded-[2.5rem] p-8 text-white space-y-10 shadow-5xl border border-white/5 relative overflow-hidden">
                   <div className="absolute top-0 right-0 p-12 opacity-5"><Database className="h-64 w-64" /></div>
                   <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8">
                      <div className="space-y-3"><Label className="text-[11px] font-black uppercase text-slate-500 ml-1">Authority Center</Label><select value={filterBoard} onChange={e => setFilterBoard(e.target.value)} className="w-full h-11 md:h-12 bg-white/5 border border-white/10 rounded-full px-4 outline-none font-bold uppercase text-[11px]"><option value="all" className="bg-[#0B1528]">All Boards</option>{boards?.map((b: any) => <option key={b.id} value={b.id} className="bg-[#0B1528]">{b.abbreviation}</option>)}</select></div>
                      <div className="space-y-3"><Label className="text-[11px] font-black uppercase text-slate-500 ml-1">Subject Node</Label><select value={filterSubject} onChange={e => setSubjectFilter(e.target.value)} className="w-full h-11 md:h-12 bg-white/5 border border-white/10 rounded-full px-4 outline-none font-bold uppercase text-[11px]"><option value="all" className="bg-[#0B1528]">All Nodes</option>{subjects?.map((s: any) => <option key={s.id} value={s.id} className="bg-[#0B1528]">{s.name}</option>)}</select></div>
                      <div className="flex flex-col justify-end space-y-3"><div className="flex items-center justify-between bg-white/5 px-5 py-2.5 rounded-full border border-white/10 h-11 md:h-12"><span className="text-[10px] font-black uppercase tracking-widest text-slate-300">UNUSED ONLY</span><Switch checked={hideUsed} onCheckedChange={setHideUsed} /></div></div>
                   </div>
                   <div className="relative z-10 flex flex-col md:flex-row items-center gap-4 pt-6 border-t border-white/10"><div className="flex-1 text-left"><p className="text-[10px] font-black uppercase text-blue-400 tracking-[0.3em]">Selection Queue</p><div className="text-xl md:text-3xl font-headline font-black tabular-nums">{bankSelection.length} <span className="text-sm text-slate-500">Nodes Staged</span></div></div><Button onClick={handleLinkQuestions} disabled={bankSelection.length === 0} variant="emerald" size="lg" className="w-full md:w-auto gap-3">Link Staged Assets <CheckCircle2 className="h-5 w-5" /></Button></div>
                </Card>
                <div className="grid grid-cols-1 gap-3">
                   {bankLoading ? Array.from({length: 5}).map((_, i) => <Skeleton key={i} className="h-20 w-full rounded-[1.5rem] bg-white" />) : visibleBank.map((q: Question) => {
                      const isSelected = bankSelection.includes(q.id);
                      return (<Card key={q.id} onClick={() => setBankSelection((p: string[]) => isSelected ? p.filter(id => id !== q.id) : [...p, q.id])} className={cn("border-none shadow-sm rounded-2xl p-5 md:px-8 flex items-center justify-between cursor-pointer transition-all border-2", isSelected ? "bg-blue-600/5 border-blue-600 shadow-lg" : "bg-white border-transparent hover:border-slate-100")}><div className="flex items-center gap-6 min-w-0"><div className={cn("h-6 w-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all", isSelected ? "bg-blue-600 border-blue-600" : "bg-white border-slate-200")}>{isSelected && <Check className="h-3 w-3 text-white stroke-[4px]" />}</div><div className="min-w-0 text-left"><p className="font-bold text-[#0F172A] truncate text-sm md:text-base">{q.englishQuestion}</p><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest mt-1.5">{q.subjectId} • {q.difficulty}</p></div></div></Card>)
                   })}
                   {filteredBank.length > displayLimit && <Button variant="ghost" onClick={() => setDisplayLimit((d: number) => d + 100)} className="w-full">Load More ({filteredBank.length - displayLimit} Left)</Button>}
                </div>
             </div>
           ) : (
             <div className="space-y-8 animate-in fade-in duration-500">
                <div className="flex flex-col md:flex-row items-center justify-between px-2 gap-4">
                   <div className="text-left"><h3 className="text-xl font-headline font-black uppercase text-[#0F172A] flex items-center gap-3"><Layers className="h-6 w-6 text-blue-600" /> Active Composition</h3></div>
                   <Popover>
                      <PopoverTrigger asChild><Button variant="dark" className="gap-3"><Plus className="h-4 w-4" /> Add Section Node</Button></PopoverTrigger>
                      <PopoverContent className="w-[320px] p-6 bg-[#0F172A] text-white rounded-[2rem] border-white/10 shadow-5xl z-[1001]">
                         <div className="space-y-4">
                            <Label className="text-[10px] font-black uppercase text-blue-400 ml-1">Section hub Name</Label>
                            <Input placeholder="e.g. PUNJAB GK" className="h-12 bg-white/5 border-white/10 text-white rounded-full font-bold px-6" onKeyDown={(e) => {
                               if(e.key === 'Enter') {
                ...
                <div className="grid grid-cols-1 gap-8">
                   {sections.map((sec: any, sIdx: number) => (
                      <Card key={sec.id} className="border-none shadow-3xl rounded-[2.5rem] bg-white overflow-hidden border border-slate-100 group/sec">
                         <div className="flex items-center justify-between p-8 bg-slate-50/50 border-b border-slate-50"><div className="flex items-center gap-6"><div className="h-12 w-12 rounded-2xl flex items-center justify-center font-black text-xl shadow-xl bg-[#0F172A] text-white">{sIdx + 1}</div><div className="text-left"><Input value={sec.name} onChange={e => setSections((p: any[]) => p.map((s: any) => s.id === sec.id ? { ...s, name: e.target.value.toUpperCase() } : s))} className="h-8 p-0 bg-transparent border-none font-black uppercase text-xl focus-visible:ring-0 text-[#0F172A]" /><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{(sec.questions?.length || 0)} Prep Nodes</p></div></div><div className="flex gap-2"><Button onClick={() => setActiveSectionId(sec.id)} variant={activeSectionId === sec.id ? "default" : "ghost"} size="sm">{activeSectionId === sec.id ? "ACTIVE" : "FOCUS"}</Button><Button variant="ghost" size="icon" onClick={() => setSections((p: any[]) => p.filter((s: any) => s.id !== sec.id))} className="text-rose-500 hover:bg-rose-50 rounded-full"><Trash2 className="h-4 w-4" /></Button></div></div>
