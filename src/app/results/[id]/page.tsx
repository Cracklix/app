...
/**
 * @fileOverview Test Results Hub v2.0 (High-Fidelity).
 * MATCHED: Design strictly follows Testbook/Gradeup inspired elite layouts.
 * FIXED: NaN errors resolved with proper data fallbacks.
 */
...
export default function ResultPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 font-body pb-safe text-left">
      ...
      <main className="container mx-auto px-4 md:px-6 py-8 md:py-16 max-w-7xl space-y-12 md:space-y-20">
        
        {/* HERO HUB (Testbook Professional Style) */}
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12">
           <Card className="flex-1 border-none shadow-5xl rounded-[3rem] md:rounded-[4rem] bg-[#0B1528] text-white overflow-hidden relative group">
                 ...
                 <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
                    <div className="space-y-6">
                       <div className="flex items-center gap-4">
                          ...
                          <Badge className="bg-primary/20 text-primary border-none px-5 py-2 rounded-full font-black uppercase text-[10px] tracking-[0.2em] shadow-xl">Audit Finalized</Badge>
                       </div>
                       <h1 className="text-4xl md:text-7xl font-headline font-black uppercase leading-[0.85] tracking-tighter">
                          {sessionData.mockTitle}
                       </h1>
                    </div>
                    
                    <div className="flex items-center gap-8 md:gap-16 bg-white/5 backdrop-blur-3xl p-10 md:p-16 rounded-[4rem] border border-white/10 shadow-5xl group/merit">
                       <div className="text-center space-y-2">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">RANK</p>
                          <p className="text-6xl md:text-9xl font-headline font-black text-primary leading-none text-center tabular-nums group-hover/merit:scale-105 transition-transform duration-500">#{merit.rank}</p>
                          <p className="text-[11px] font-black text-slate-500 uppercase text-center">OUT OF {merit.total}</p>
                       </div>
                       <div className="h-24 w-px bg-white/10" />
                       <div className="text-center space-y-2">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">EFFICIENCY</p>
                          <p className="text-6xl md:text-9xl font-headline font-black text-emerald-400 leading-none text-center tabular-nums">{merit.percentile}</p>
                          <p className="text-[11px] font-black text-slate-500 uppercase text-center">PERCENTILE</p>
                       </div>
                    </div>
                 </div>

                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-10 pt-10 border-t border-white/5">
           ...
           <div className="w-full lg:w-96 flex flex-col gap-6 md:gap-8">
              <Card className="border-none shadow-3xl rounded-[3rem] bg-white p-10 space-y-8 border border-slate-100">
                 <div className="flex items-center gap-3">
                    <Zap className="h-5 w-5 text-primary" />
                    <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Tactical Actions</h3>
                 </div>
                 <div className="space-y-4">
                    <Button onClick={() => window.print()} className="w-full h-16 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] shadow-2xl shadow-emerald-500/10 gap-3 transition-all active:scale-95 border-none">
                       <Download className="h-5 w-5" /> Download Report
                    </Button>
                    <Button variant="outline" asChild className="w-full h-16 border-2 border-slate-100 hover:border-primary hover:text-primary rounded-2xl font-black uppercase text-[11px] tracking-[0.2em] text-slate-600 transition-all active:scale-95">
              ...
              {profile?.status === 'Free' && (
                <div className="bg-primary rounded-[3rem] p-12 text-white relative overflow-hidden group shadow-4xl cursor-pointer">
                   ...
                   <div className="relative z-10 space-y-6">
                      <h4 className="text-3xl font-headline font-black uppercase leading-[1.1]">Join the <br/> Elite Cohort</h4>
                      <p className="text-white/70 text-[10px] font-bold uppercase tracking-widest">Unlock AI rationales and all state ranks.</p>
                      <Button asChild className="w-full h-14 bg-white text-primary hover:bg-slate-50 font-black rounded-xl text-[10px] uppercase tracking-widest shadow-xl border-none">
        ...
        <Tabs defaultValue="SECTIONAL" className="space-y-12">
           <div className="bg-white border border-slate-100 p-1.5 h-20 rounded-[2rem] md:rounded-[3rem] shadow-xl inline-flex w-full md:w-auto overflow-x-auto no-scrollbar justify-start gap-3 px-3">
              <TabsTrigger value="SECTIONAL" className="rounded-2xl px-10 font-black uppercase text-[10px] tracking-widest gap-3 h-full data-[state=active]:bg-[#0B1528] data-[state=active]:text-white data-[state=active]:shadow-2xl transition-all whitespace-nowrap">
                 <BarChart3 className="h-4 w-4" /> Sectional Audit
              </TabsTrigger>
              <TabsTrigger value="TOPPER" className="rounded-2xl px-10 font-black uppercase text-[10px] tracking-widest gap-3 h-full data-[state=active]:bg-[#0B1528] data-[state=active]:text-white data-[state=active]:shadow-2xl transition-all whitespace-nowrap">
                 <Trophy className="h-4 w-4" /> State Comparison
              </TabsTrigger>
              <TabsTrigger value="SOLUTIONS" className="rounded-2xl px-10 font-black uppercase text-[10px] tracking-widest gap-3 h-full data-[state=active]:bg-[#0B1528] data-[state=active]:text-white data-[state=active]:shadow-2xl transition-all whitespace-nowrap">
           ...
           <TabsContent value="SECTIONAL" className="m-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                 {sectionalAnalysis.map((s, i) => (
                    <Card key={i} className="border-none shadow-2xl rounded-[3rem] bg-white p-10 group hover:translate-y-[-8px] transition-all border border-slate-100 hover:border-primary/20">
                       <CardHeader className="p-0 mb-10 flex flex-row items-center justify-between">
                          <div className="space-y-1 text-left min-w-0 flex-1 pr-4">
                             <h4 className="font-headline font-black text-2xl uppercase text-[#0B1528] leading-none truncate">{s.name}</h4>
                             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Sectional Index</p>
                          </div>
                          <Badge className={cn(
                             "border-none text-[10px] font-black uppercase px-4 py-1.5 rounded-xl shadow-lg", 
                       ...
                       <div className="space-y-10">
                          <div className="grid grid-cols-2 gap-8">
                             <div className="text-left bg-slate-50/50 p-6 rounded-3xl border border-slate-100/50 shadow-inner">
                                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2">MARKS</p>
                                <p className="text-4xl font-headline font-black text-[#0B1528]">{s.score.toFixed(1)}</p>
                             </div>
                             <div className="text-left bg-rose-50/30 p-6 rounded-3xl border border-rose-100/50">
                                <p className="text-[9px] font-black text-rose-300 uppercase tracking-widest mb-2">WRONG</p>
                                <p className="text-4xl font-headline font-black text-rose-600">{s.wrong}</p>
                             </div>
                          </div>
                          <div className="space-y-4">
           ...
           <TabsContent value="TOPPER" className="m-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <Card className="border-none shadow-5xl rounded-[4rem] bg-white overflow-hidden border border-slate-100">
                 <div className="grid grid-cols-1 lg:grid-cols-2">
                    <div className="p-12 md:p-24 space-y-16 border-b lg:border-b-0 lg:border-r border-slate-50">
                       <div className="flex items-center gap-8">
                          <div className="h-20 w-20 rounded-[2rem] bg-amber-50 flex items-center justify-center text-amber-500 shadow-xl border border-amber-100">
                             <Trophy className="h-10 w-10" />
                          </div>
                          <div className="space-y-1 text-left">
                             <h3 className="font-headline font-black text-4xl uppercase text-[#0B1528] tracking-tight">Merit Benchmark</h3>
                             <p className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-400">Comparing your node with state rank #1</p>
                          </div>
                       </div>
                    ...
                    <div className="p-12 md:p-24 flex flex-col items-center justify-center text-center space-y-12 bg-slate-50/50 relative overflow-hidden">
                       <div className="absolute top-[-10%] right-[-10%] opacity-5"><Target className="h-96 w-96 text-[#0B1528]" /></div>
                       <div className="relative group">
                          <StudentAvatar profile={merit.topper} className="h-48 w-48 md:h-72 md:w-72 border-[16px] border-white shadow-5xl rounded-[5rem] transition-transform duration-700 group-hover:scale-105" />
                          <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 bg-amber-400 text-white px-10 py-3 rounded-full font-black text-[12px] uppercase shadow-3xl tracking-[0.3em] border-4 border-white animate-bounce">STATE TOPPER</div>
                       </div>
                       <div className="space-y-4 relative z-10">
                          <p className="text-primary font-black uppercase tracking-[0.5em] text-[11px]">MASTER HUB LEADER</p>
                          <h4 className="text-4xl md:text-5xl font-headline font-black uppercase text-[#0B1528] tracking-tight">{merit.topper?.name || 'Academic Topper'}</h4>
                       </div>
                    </div>
                 </div>
              </Card>
           ...
           <TabsContent value="SOLUTIONS" className="m-0 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="bg-white border border-slate-100 rounded-[3rem] p-8 shadow-xl flex flex-wrap items-center gap-6 sticky top-24 z-30 backdrop-blur-xl bg-opacity-95">
              ...
              <div className="grid grid-cols-1 gap-10">
                 {filteredQuestions.map((q) => {
                    ...
                    return (
                       <Card key={q.id} className="border-none shadow-3xl rounded-[3.5rem] overflow-hidden bg-white group hover:shadow-5xl transition-all border border-slate-100 relative text-left">
                          ...
                          <CardContent className="p-10 md:p-20 space-y-12">
                             <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
                                <div className="flex items-center gap-10">
                                   <div className={cn(
                                      "h-16 w-16 md:h-24 md:w-24 rounded-3xl flex items-center justify-center font-black text-3xl md:text-5xl shadow-inner transition-transform group-hover:rotate-6",
                                   ...
                                   <div className="space-y-2 text-left">
                                      <div className="flex items-center gap-3">
                                         <Badge className={cn(
                                            "border-none text-[11px] font-black uppercase px-5 py-2 rounded-xl shadow-lg", 
                                            isCorrect ? 'bg-emerald-50 text-emerald-600' : isSkipped ? 'bg-slate-100 text-slate-400' : 'bg-rose-50 text-rose-600'
                                         )}>
                                            {isCorrect ? 'CORRECT' : isSkipped ? 'SKIPPED' : 'INCORRECT'}
                                         </Badge>
                                         {!isCorrect && !isSkipped && <Badge className="bg-[#0B1528] text-white border-none text-[11px] font-black uppercase px-4 py-2 rounded-xl">Correct: {q.correctAnswer}</Badge>}
                                      </div>
                                      <p className="text-[12px] font-black text-slate-400 uppercase tracking-[0.3em] ml-1">{q.sectionId || 'GENERAL HUB'}</p>
                                   </div>
                                </div>
                                <Button 
                                   onClick={() => setExpandedQs(prev => ({ ...prev, [q.index]: !prev[q.index] }))}
                                   variant="ghost" 
                                   className="h-16 px-12 rounded-[1.5rem] font-black uppercase text-[11px] tracking-[0.2em] gap-4 bg-slate-50 text-[#0B1528] hover:bg-[#0B1528] hover:text-white transition-all shadow-xl active:scale-95 shrink-0"
                                >
                                   {isExpanded ? 'Hide Logic' : 'View AI Rationale'}
                                   {isExpanded ? <ChevronUp className="h-6 w-6" /> : <BrainCircuit className="h-6 w-6 text-primary" />}
                                </Button>
                             </div>
...
function MetricCard({ label, val, sub, color }: any) {
   return (
      <div className="space-y-4 p-8 md:p-12 bg-white/5 border border-white/5 rounded-[3.5rem] transition-all hover:bg-white/10 group shadow-2xl text-left cursor-pointer">
         <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">{label}</p>
         <p className={cn("text-4xl md:text-7xl font-headline font-black leading-none tracking-tighter", color)}>{val}</p>
         <div className="flex items-center gap-2 pt-2">
            <div className={cn("h-1.5 w-1.5 rounded-full animate-pulse", val === 'NaNm' ? 'bg-rose-500' : 'bg-emerald-500')} />
      ...
      <div className="space-y-6 text-left">
         <div className="flex justify-between items-end">
            <div className="space-y-1">
               <span className="text-[12px] font-black uppercase tracking-[0.3em] text-[#0B1528]">{label}</span>
               <div className="h-1 w-8 bg-primary rounded-full" />
            </div>
            <div className="flex gap-10">
               <div className="text-right">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">YOUR HUB</p>
                  <p className="text-2xl font-black text-[#0B1528]">{user}{unit}{isTime && 'm'}</p>
               </div>
               <div className="text-right">
                  <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">TOPPER</p>
                  <p className="text-2xl font-black text-amber-600">{topper}{unit}{isTime && 'm'}</p>
               </div>
            </div>
         </div>
         <div className="relative h-4 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner">
            ...
            <div className="absolute inset-0 bg-amber-400 transition-all duration-[1800ms] ease-out border-r-4 border-amber-600" style={{ width: `${topperPer}%` }} />
...
function FilterPill({ active, label, count, onClick, color }: any) {
   return (
      <button 
         onClick={onClick}
         className={cn(
            "flex items-center gap-5 px-10 py-5 rounded-[1.5rem] transition-all active:scale-95 border-2 shadow-sm",
      ...
      >
         <span className={cn("h-8 w-8 rounded-xl flex items-center justify-center font-black text-sm shadow-md", color)}>{count}</span>
