...
export default function ExamHubPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50/50 font-body">
      ...
      {/* Exam Context Hero */}
      <section className="bg-[#0B1528] text-white py-12 md:py-24 relative overflow-hidden">
            ...
            <div className="max-w-4xl space-y-6">
               <div className="flex flex-wrap items-center gap-3">
                  <Badge className="bg-primary text-white border-none px-4 py-1.5 rounded-lg font-black uppercase text-[10px] tracking-[0.2em]">
                     Official {exam.boardId || "State"} Hub
                  </Badge>
               ...
               <h1 className="text-4xl md:text-7xl font-headline font-black leading-[0.95] tracking-tight uppercase">
                  {exam.name} <br/> <span className="text-primary">Mastery Node</span>
               </h1>
      ...
      <main className="container mx-auto px-4 -mt-10 max-w-7xl relative z-20 pb-32">
         <Tabs defaultValue="FULL" className="space-y-12">
            <div className="bg-white p-1.5 md:p-2 rounded-[1.5rem] md:rounded-[2.5rem] shadow-4xl border border-slate-100 flex items-center overflow-x-auto no-scrollbar">
               <TabsList className="bg-transparent border-none p-0 flex gap-1 h-auto">
                  ...
                  <TabTrigger value="CHAPTER" icon={<Layout />} label="Chapter" count={groupedMocks.CHAPTER.length} />
                  <TabTrigger value="PYQ" icon={<FileText />} label="PYQs" count={groupedMocks.PYQ.length} />
                  <TabTrigger value="CA_QUIZ" icon={<Newspaper />} label="CA Quiz" count={groupedMocks.CA_QUIZ.length} />
               </TabsList>
            </div>
            ...
            <TabsContent value="CHAPTER" className="space-y-6">
               <HubGrid mocks={groupedMocks.CHAPTER} emptyLabel="Chapter-wise practice node is currently offline." />
            </TabsContent>
            ...
            <TabsContent value="CA_QUIZ" className="space-y-6">
               <HubGrid mocks={groupedMocks.CA_QUIZ} emptyLabel="Exam-specific CA quizzes will appear here." />
            </TabsContent>
         </Tabs>
      </main>
...
function TabTrigger({ value, icon, label, count }: any) {
   return (
      <TabsTrigger 
         value={value} 
         className="rounded-xl md:rounded-[1.5rem] px-4 md:px-8 h-12 md:h-16 font-black uppercase text-[9px] md:text-[11px] tracking-widest data-[state=active]:bg-[#0F172A] data-[state=active]:text-white flex items-center gap-2 md:gap-3 transition-all"
      >
         <span className="shrink-0">{icon}</span>
         <span className="whitespace-nowrap">{label}</span>
         <Badge className="bg-primary/10 text-primary border-none text-[8px] font-black group-data-[state=active]:bg-white/10 group-data-[state=active]:text-white">{count}</Badge>
      </TabsTrigger>
   )
}
...
function MockCard({ mock }: { mock: any }) {
  return (
    <Card className="border-none shadow-xl shadow-slate-200/40 hover:shadow-4xl transition-all duration-500 rounded-[2.5rem] bg-white group overflow-hidden text-left flex flex-col h-full border border-slate-50">
      <CardContent className="p-0 flex-1 flex flex-col h-full">
         <div className="p-8 pb-4 space-y-6 flex-1">
            ...
            <div className="space-y-4">
               <h3 className="font-headline text-lg md:text-2xl font-black text-[#000000] leading-tight uppercase group-hover:text-primary transition-colors min-h-[56px] line-clamp-2">
                {mock.title}
               </h3>
               <div className="flex flex-col gap-2.5">
                  ...
                  <div className="flex items-center gap-2 text-slate-400 font-black uppercase text-[9px] tracking-widest">
                     <History className="h-3.5 w-3.5 text-primary" /> {mock.year || '2026'} Series
                  </div>
               </div>
            </div>
         </div>
         ...
         <div className="p-6 pt-0 mt-auto">
            <Button asChild className="w-full h-12 md:h-14 bg-[#0B1528] hover:bg-primary text-white font-black uppercase tracking-[0.2em] text-[10px] rounded-2xl shadow-xl transition-all active:scale-95 group">
               <Link href={`/mocks/${mock.id}`} className="flex items-center justify-center gap-3">
                  Start Practice <ChevronRight className="h-4 w-4 group-hover:translate-x-2 transition-transform" />
               </Link>
            </Button>
         </div>
      </CardContent>
   ...
   return (
      <div className="flex items-center gap-3">
         ...
         <div className="space-y-0.5">
            <p className="text-lg font-headline font-black leading-none">{value}</p>
...
function EmptyState({ label }: { label: string }) {
  return (
    <div className="col-span-full py-32 text-center text-slate-300 border-2 border-dashed border-slate-200 rounded-[3.5rem] space-y-6 bg-white/50">
       ...
       <div className="space-y-1 px-10">
          ...
          <p className="text-xs font-bold uppercase tracking-widest text-slate-400">Preparation content for this node is currently being audited.</p>
       </div>
    </div>
  )
}
