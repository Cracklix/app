...
/**
 * @fileOverview Canonical Punjab Exam List v15.0.
 */
             ...
             <Link key={cat.id} href={`/exams/category/${cat.id}`}>
                <Card className="border-none shadow-xl hover:shadow-4xl transition-all duration-500 rounded-[2rem] bg-white group overflow-hidden h-full flex flex-col p-6">
                   <div className="flex justify-between items-start mb-6">
                      ...
                      <Badge className="bg-[#0F172A] text-white border-none text-[8px] font-black uppercase px-3 py-1 rounded-lg">{cat.highlight || "VERTICAL"}</Badge>
                   </div>
                   ...
                   <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between">
                      <Button variant="ghost" className="h-10 px-6 rounded-xl bg-[#0F172A] text-white group-hover:bg-primary transition-all font-bold text-[10px] tracking-widest uppercase border-none shadow-md">Open Category <ChevronRight className="ml-2 h-3.5 w-3.5" /></Button>
                   </div>
                </Card>
             </Link>
           ))}
