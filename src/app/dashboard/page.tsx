...
export default function StudentDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-safe text-left">
      ...
      <main className="container mx-auto px-4 md:px-8 py-6 md:py-10 max-w-7xl space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
          
          <div className="lg:col-span-8 space-y-8">
              <section className="bg-[#0B1528] text-white p-6 md:p-10 rounded-[2rem] shadow-2xl relative overflow-hidden group text-left border border-white/5">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-6 md:gap-10">
                  <div className="relative shrink-0">
                    {profileLoading && !profile ? (
                      <Skeleton className="h-20 w-20 md:h-28 md:w-28 rounded-2xl md:rounded-3xl bg-white/5" />
                    ) : (
                      <div className="relative">
                        <StudentAvatar profile={profile} className="h-20 w-20 md:h-28 md:w-28 border-[3px] border-white/10 rounded-2xl md:rounded-3xl shadow-5xl bg-[#0F172A]" />
                        <div className="absolute -bottom-1 -right-1 bg-emerald-500 h-6 w-6 md:h-8 md:w-8 rounded-lg border-2 border-[#0B1528] flex items-center justify-center text-white shadow-xl">
                  ...
                  <div className="flex-1 space-y-3 text-center md:text-left min-w-0">
                    <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                          <div className={cn(
                            "flex items-center gap-2 px-3 py-1 rounded-full font-black uppercase text-[8px] md:text-[9px] tracking-widest transition-all",
                            isActive ? "bg-primary text-white" : "bg-white/10 text-slate-400"
                          )}>
                             <Gem className="h-3 w-3" />
                             {isActive ? (passCountdown || 'ACTIVE') : 'FREE PASS'}
                          </div>
                          <div className="text-slate-400 font-bold uppercase tracking-widest text-[8px] md:text-[10px] flex items-center gap-2">
                            <Target className="h-3.5 w-3.5 text-primary" /> {profile?.targetExam || 'Punjab Exams'}
                          </div>
                        </div>
                    </div>
                  </div>
              ...
              <Card className="border-none shadow-xl rounded-[1.5rem] md:rounded-[2rem] bg-white overflow-hidden text-left border border-slate-100">
                    ...
                    <div className="divide-y divide-slate-50">
                      {resultsLoading ? (
                          ...
                          stats.list.map((r: any) => (
                            <Link key={r.id} href={`/results/${r.mockId}`} className="p-5 md:p-8 flex items-center justify-between hover:bg-slate-50/50 transition-all group">
                                <div className="flex items-center gap-4 md:gap-6 min-w-0 flex-1">
                                  <div className="h-9 w-9 md:h-12 md:w-12 rounded-xl bg-slate-50 flex items-center justify-center shrink-0 shadow-inner">
...
function MetricItem({ label, val, icon }: any) {
  return (
    <Card className="border-none shadow-lg bg-white p-4 md:p-6 rounded-[1.25rem] text-left group hover:translate-y-[-2px] transition-all border border-slate-100 min-w-0">
      <div className="h-8 w-8 rounded-lg bg-slate-50 flex items-center justify-center mb-4 group-hover:bg-primary/5 transition-all shadow-inner shrink-0">
   ...
   return (
      <Link href={href} className="flex items-center justify-between p-4 rounded-xl border border-slate-50 bg-slate-50/50 hover:bg-slate-100 transition-all active:scale-[0.98] group">
         <div className="flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-[#0F172A] uppercase tracking-tight">{label}</span>
         </div>
