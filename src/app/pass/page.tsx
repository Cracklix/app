...
export default function PassPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020817] font-body pb-safe overflow-x-hidden text-white">
      ...
      <main className="container mx-auto px-4 py-16 md:py-24 max-w-7xl relative z-10 text-left">
        <div className="text-center space-y-6 mb-16 md:mb-24">
           <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              <p className="text-sm md:text-xl text-slate-400 font-medium max-w-2xl mx-auto mt-8 leading-relaxed">
                 Unlock all premium mock tests, verified answer keys, and AI performance analytics to secure your selection.
              </p>
           </motion.div>
        </div>
             ...
             <motion.div key={plan.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.1 }}>
                <Card className={cn(
                  ...
                  plan.id.includes('quarterly') ? "bg-[#0B1528] ring-2 ring-primary/40" : "bg-white/5 border border-white/10"
                   ...
                   <CardHeader className="p-12 pb-8 text-center space-y-8">
                      <div className={cn("h-20 w-20 rounded-[2.5rem] flex items-center justify-center mx-auto shadow-2xl", plan.id.includes('quarterly') ? "bg-primary text-white" : "bg-white/10 text-primary")}>
                         <Gem className="h-10 w-10 fill-current" />
                      </div>
                      <div className="space-y-2">
                        <CardTitle className="font-headline font-black text-3xl uppercase tracking-tight text-white leading-tight">
                           {plan.name}
                        </CardTitle>
                      </div>
                      <div className="flex items-baseline justify-center gap-2">
                         <span className="text-6xl font-headline font-black text-white tracking-tighter">₹{plan.price}</span>
                         <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">/ {plan.durationDays} Days</span>
                      </div>
                   </CardHeader>

                   <CardContent className="px-12 pb-10 flex-1">
                   ...
                   <CardFooter className="p-12 pt-0">
                      <Button asChild className="w-full h-16 rounded-[2rem] bg-primary hover:bg-orange-600 text-white font-black uppercase text-[11px] tracking-[0.2em] shadow-3xl transition-all active:scale-95 border-none">
        ...
        <div className="mt-32 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
           <div className="space-y-8">
              <div className="flex items-center gap-4">
                 ...
                 <h2 className="text-3xl md:text-5xl font-headline font-black uppercase text-white">Manual Hub <br/> Verification</h2>
              </div>
              <p className="text-slate-400 text-lg font-medium leading-relaxed max-w-xl">
                 Secure your pass via direct institutional transaction nodes. Simply pay using the QR/UPI ID on the checkout page and upload your 12-digit UTR ID for rapid registry audit.
              </p>
           </div>
           <div className="grid grid-cols-2 gap-4">
              <BenefitNode label="High Fidelity Mocks" icon={<ShieldCheck className="text-emerald-500" />} />
              <BenefitNode label="Official PYQ Access" icon={<Zap className="text-amber-500" />} />
              <BenefitNode label="State Merit Index" icon={<Star className="text-primary" />} />
              <BenefitNode label="AI Rationalization" icon={<Sparkles className="text-blue-500" />} />
           </div>
        </div>
      </main>
      <Footer />
...
function BenefitNode({ label, icon }: any) {
   return (
      <div className="p-8 bg-white/5 border border-white/5 rounded-[2.5rem] space-y-4 shadow-inner hover:bg-white/10 transition-all group">
         <div className="h-12 w-12 rounded-xl bg-white/5 flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform">{icon}</div>
