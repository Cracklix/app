...
/**
 * @fileOverview Student Profile Center v23.0 (Optimized Loading).
 */
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">Loading Profile Node...</p>
      </div>
    )
  }
           ...
           <div className="container mx-auto px-4 md:px-6 max-w-6xl pt-6 md:pt-16 pb-12 md:pb-20">
              <div className="flex flex-col md:flex-row items-center md:items-end gap-5 md:gap-10 relative z-10">
                 <div className="relative group shrink-0">
                    {profileLoading ? (
                      <Skeleton className="h-20 w-20 md:h-32 md:w-32 rounded-[2rem] md:rounded-[2.5rem] bg-white/5" />
                    ) : (
                 ...
                 <div className="flex-1 space-y-2 md:space-y-3 min-w-0 text-center md:text-left">
                    {profileLoading ? (
                      <div className="space-y-4">
                        <Skeleton className="h-8 md:h-12 w-1/2 bg-white/5 mx-auto md:mx-0" />
                        <Skeleton className="h-4 w-1/3 bg-white/5 mx-auto md:mx-0" />
                      </div>
                    ) : (
                      <>
                 ...
                 <div className="shrink-0 w-full md:w-auto pt-3 md:pt-0 flex flex-col sm:flex-row gap-2 md:gap-3">
                    <Button onClick={() => setIsEditing(true)} disabled={profileLoading} className="h-10 md:h-14 px-6 md:px-8 bg-white/10 hover:bg-white/20 text-white border border-white/10 rounded-xl font-black uppercase text-[9px] md:text-[10px] tracking-widest gap-2 shadow-2xl"><Edit className="h-3.5 w-3.5 text-primary" /> Edit</Button>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 md:gap-10">
              <div className="lg:col-span-8 space-y-5 md:space-y-8">
                 <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-6">
                    ...
                    <StatsCard icon={<Target />} label="AVG ACCURACY" value={resultsLoading ? "..." : `${stats.avgAccuracy}%`} color="text-primary" bgColor="bg-primary/10" />
                 ...
                 <Card className="border-none shadow-xl rounded-[2rem] md:rounded-[2.5rem] bg-white p-6 md:p-10 space-y-6 md:space-y-8 text-left">
                    ...
                    {profileLoading ? (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6 md:gap-y-8">
                        ...
                        <ProfileDataNode icon={<Activity className="text-orange-500" />} label="JOINED" value={profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString('en-GB', { month: 'long', year: 'numeric' }) : "---"} />
                      </div>
                    )}
                 </Card>
              </div>
         ...
         <DialogContent className="sm:max-w-xl w-[95vw] max-h-[90vh] bg-white rounded-[2.5rem] border-none shadow-5xl p-0 overflow-hidden text-left flex flex-col">
            ...
            <div className="px-6 md:px-8 pb-6 md:pb-8 space-y-4 md:space-y-5 overflow-y-auto custom-scrollbar flex-1">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  ...
                  <div className="space-y-1.5 text-left">
                     <Label className="text-[9px] font-black uppercase text-slate-500 ml-1">Target Board</Label>
