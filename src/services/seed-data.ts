...
/**
 * @fileOverview Official Institutional Registry Seeder v54.0.
 * RESTRUCTURED: Moved SSC and UPSC to Central Government.
 * MIGRATION: Auto-maps Teaching exams (ETT, Professor) to the correct vertical.
 */
...
export async function seedInitialData(db: Firestore) {
  ...
  // 3. AUTO-MIGRATION REPAIR NODE
  console.log('[MIGRATION] Scanning for teaching-related vertical mismatches...');
  const examsSnap = await getDocs(collection(db, "exams"));
  examsSnap.docs.forEach((examDoc) => {
     const data = examDoc.data();
     const name = (data.name || "").toLowerCase();
     
     // Teaching Keyword Mapping
     const isTeaching = 
        name.includes("ett") || 
        name.includes("professor") || 
        name.includes("master cadre") || 
        name.includes("lecturer") || 
        name.includes("pstet") ||
        name.includes("teaching");

     if (isTeaching && data.categoryId !== "punjab-teaching-exams") {
        console.log(`[RE-MAPPING] ${data.name} -> Teaching Category`);
  ...
  await batch.commit();
  console.log('[SUCCESS] Registry Synchronized and Exams Repaired.');
}
